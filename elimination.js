...
class Elimination {
    ...
    setIndex(indexArr) {
        ...
        this.indexData.top = [-1, indexArr[1]]//初始化对应的top
        this.indexData.bottom = [-1, indexArr[1]]//初始化对应的bottom
        this.getRowDatas()//更新对应的left right  0 跳过
        this.getColDatas()//更新对应的top bottom  0 跳过
    }
    /**
     * 洗牌 随机打乱当前已存在的二维数组
     * @returns 
     ...
     */
    getElement(type) {
        if(Object.keys(this.indexData).includes(type) === false) return -1;//判断是否包含该类型 left right top bottom
    ...
    isrRemove() {
        return {
            ...
            top: [...this.indexData.top,this.getElement('top') === this.getElement('click')],
            bottom: [...this.indexData.bottom,this.getElement('bottom') === this.getElement('click')]
        }
    }
    /**
    ...
    //判断同一列是否还有可消除的
    getColDatas() {
        this.indexData.top[0] = this.indexData.click[0] - 1 >= 0 ? this.indexData.click[0] - 1 : -1
        this.indexData.bottom[0] = this.indexData.click[0] + 1 < this.twoDimensionalArr.length ? this.indexData.click[0] + 1 : -1
        //向上移动指针
        while (this.indexData.top[0] >= 0 && this.getElement('top') === 0) {
            this.indexData.top[0] = this.indexData.top[0] - 1 >= 0 ? this.indexData.top[0] - 1 : -1
        }
        //向下移动指针
        while (this.indexData.bottom[0] >= 0 && this.getElement('bottom') === 0) {
            this.indexData.bottom[0] = this.indexData.bottom[0] + 1 < this.twoDimensionalArr.length ? this.indexData.bottom[0] + 1 : -1
        }
        // console.log(this.twoDimensionalArr)
        // console.log(this.indexData)
     }
